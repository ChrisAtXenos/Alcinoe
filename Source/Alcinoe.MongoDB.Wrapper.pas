...
begin
  ...
  case log_level of
    ...
    mongoc_log_level_t.MONGOC_LOG_LEVEL_WARNING: begin
      // Suppress the warning libmongoc emits when abortTransaction is sent for a
      // transaction the server has already aborted on its own:
      //
      //   Error in abortTransaction: Transaction with { txnNumber: 30 } has been aborted.
      //
      // The server aborts a transaction as soon as any statement inside it fails
      // (duplicate-key error, write conflict, document validation, lifetime
      // timeout, ...), but the client cannot know it: the session state stays
      // "in progress" driver-side, so mongoc_client_session_abort_transaction /
      // mongoc_client_session_destroy still sends the abortTransaction command
      // and the server answers with the error above. Per the driver spec the
      // abort is best-effort and its errors must be ignored, so this warning
      // carries no information: the failure that aborted the transaction was
